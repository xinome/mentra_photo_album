-- ====================================
-- ステップ2: RLSポリシー設定（循環参照修正版）
-- ====================================
-- 前提: 01-tables-only.sql が実行済み
-- 
-- 修正内容:
-- - albums と album_members の循環参照を解消
-- - 開発段階ではシンプルなポリシーを使用
-- ====================================
...
FOR INSERT WITH CHECK (true); -- アルバムオーナーがメンバーを追加できる
...
--
-- 変更点:
-- 1. albums_select: album_members参照を削除（循環参照解消）
-- 2. album_members_select: albums参照を削除（循環参照解消）
-- 3. profiles_select: 全員が読み取り可能に（プロフィールは公開情報）
--
-- 将来的に共有機能を実装する場合:
