...
**方法2: 本番環境と同じSupabaseプロジェクトを使用**

本番環境と同じデータを参照するため、セットアップが簡単です。
...
#### ローカルSupabaseの場合（方法1: 本番環境と分離）

**方法1: ローカルSupabaseを使用して本番環境と分離**

ローカル環境で独立したデータベースを使用する場合の手順です。
...
2. **ローカルSupabaseの起動**
   ...
   - anon key: （表示されたキーをコピー）

3. **データベーススキーマのセットアップ**
   - ブラウザで http://localhost:54323 を開く（Supabase Studio）
   - 左サイドバーから「SQL Editor」をクリック
   - 「New Query」をクリック
   - `supabase/sql/complete-setup.sql`の内容を**全て**コピー&ペースト
   - 「Run」ボタンをクリックして実行
   - "Success. No rows returned" と表示されれば成功

4. **ストレージバケットの確認**
   - Supabase Studio > Storage を開く
   - 「photos」バケットが作成されていることを確認
     （`complete-setup.sql`で自動作成されます）

5. **環境変数の設定**
   `.env.local`に以下を設定：
   ```env
   NEXT_PUBLIC_SUPABASE_URL=http://127.0.0.1:54321
   NEXT_PUBLIC_SUPABASE_ANON_KEY=ローカルSupabase起動時に表示されたanon-key
   ```

**ローカルSupabaseの管理コマンド**:
- 起動: `npx supabase start`
- 停止: `npx supabase stop`
- ステータス確認: `npx supabase status`
- データベースリセット: `npx supabase db reset`
