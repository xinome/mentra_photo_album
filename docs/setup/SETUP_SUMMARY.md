# セットアップ方法まとめ

## 📝 概要

SQLエラー「column "user_id" does not exist」が発生する問題に対して、**段階的セットアップ**と**手動セットアップ**の2つの方法を用意しました。

## 🎯 どの方法を選ぶべきか

### 方法1: 段階的セットアップ（推奨）⭐

**こんな人におすすめ**:
- SQLに慣れている
- エラーが出た時に対処できる
- 時間: 約5-10分

**手順**:
1. `01-tables-only.sql` → テーブル作成
2. `02-rls-policies.sql` → RLS設定
3. Storage UIからバケット作成（手動）

👉 [README.md](../../README.md#4-データベースをセットアップ) または [クイック修正ガイド](../development/QUICK_FIX_GUIDE.md)

---

### 方法2: 手動セットアップ（確実）🛡️

**こんな人におすすめ**:
- SQLエラーが解決しない
- UIから操作したい
- 確実に動作させたい
- 時間: 約15-20分

**手順**:
1. SQL Editorでテーブル作成
2. SQL EditorでRLS設定
3. Storage UIからバケット作成（手動）
4. Table Editorからサンプルデータ追加（手動）

👉 [MANUAL_SETUP_GUIDE.md](MANUAL_SETUP_GUIDE.md)

---

## 📂 ファイル一覧

### SQLスクリプト（段階的）

| ファイル | 内容 | 実行順序 | 必須 |
|---------|------|---------|------|
| `01-tables-only.sql` | テーブル構造のみ | 1番目 | ✅ 必須 |
| `02-rls-policies.sql` | RLSポリシー設定 | 2番目 | ✅ 必須 |
| `03-storage.sql` | ストレージ設定（参考） | 3番目 | ❌ 手動推奨 |
| `sample-data.sql` | サンプルデータ | 最後 | ❌ オプション |

### SQLスクリプト（統合版・非推奨）

| ファイル | 内容 | 状態 |
|---------|------|------|
| `complete-setup.sql` | 全てを一括実行 | ⚠️ エラーが出る可能性あり |

### ドキュメント

| ファイル | 内容 | 対象読者 |
|---------|------|---------|
| [手動セットアップガイド](MANUAL_SETUP_GUIDE.md) | 手動セットアップ手順 | 確実に動作させたい人 |
| [クイック修正ガイド](../development/QUICK_FIX_GUIDE.md) | クイック修正ガイド | 段階的セットアップをしたい人 |
| [データベース詳細ガイド](../database/DATABASE_SETUP.md) | データベース詳細ガイド | 詳しく知りたい人 |
| [テーブル構造リファレンス](../database/TABLE_STRUCTURE.md) | テーブル構造リファレンス | 開発者向け |
| [セットアップガイド](SETUP_GUIDE.md) | セットアップガイド | 全員 |

---

## 🚀 クイックスタート

### 初めてセットアップする場合

1. **環境変数設定**
   ```bash
   cp .env.local.example .env.local
   # .env.local を編集して Supabase URL と API キーを設定
   ```

2. **段階的セットアップ**
   - `01-tables-only.sql` を実行
   - `02-rls-policies.sql` を実行
   - Storage UI から `photos` バケット作成

3. **動作確認**
   ```bash
   npm run dev
   ```
   - `http://localhost:3000` にアクセス
   - プロフィール設定を完了
   - アルバム一覧が表示されればOK ✅

### エラーが出た場合

**症状**: `column "user_id" does not exist`

**対処法**:
1. 段階的セットアップを試す（[クイック修正ガイド](../development/QUICK_FIX_GUIDE.md)）
2. それでもダメなら手動セットアップ（[MANUAL_SETUP_GUIDE.md](MANUAL_SETUP_GUIDE.md)）

---

## ✅ セットアップ完了チェックリスト

データベースが正しくセットアップされているか確認：

### テーブル
- [ ] `profiles` テーブルが存在
- [ ] `albums` テーブルが存在
- [ ] `photos` テーブルが存在
- [ ] `shares` テーブルが存在
- [ ] `album_members` テーブルが存在
- [ ] `comments` テーブルが存在
- [ ] `photo_tags` テーブルが存在

### RLS
- [ ] すべてのテーブルでRLSが有効化されている
- [ ] `profiles` テーブルに4つのポリシーがある

### ストレージ
- [ ] `photos` バケットが存在する
- [ ] Public bucket がオフになっている

### アプリケーション
- [ ] プロフィール設定が保存できる
- [ ] アルバム一覧ページが表示される
- [ ] 新規アルバムが作成できる

**すべてチェックできたら完了です！** 🎉

---

## 🐛 よくある問題と解決方法

### 問題1: プロフィール保存エラー

```
プロフィールの保存に失敗しました
```

**原因**: テーブルまたはRLSが未設定

**解決方法**:
1. Table Editor で `profiles` テーブルが存在するか確認
2. 存在しない場合: `01-tables-only.sql` を実行
3. RLS確認: `02-rls-policies.sql` を実行

### 問題2: アルバムが表示されない

**原因**: データがない、またはRLSで弾かれている

**解決方法**:
```sql
-- SQL Editor で実行
SELECT * FROM albums WHERE owner_id = auth.uid();
```
- 結果が空: サンプルデータを追加
- エラー: RLSポリシーを再設定

### 問題3: 写真アップロードエラー

**原因**: ストレージバケットが未作成

**解決方法**:
1. Storage > Create a new bucket
2. Name: `photos`
3. Public: オフ

---

## 📚 参考リンク

- [Supabase Documentation](https://supabase.com/docs)
- [Row Level Security](https://supabase.com/docs/guides/auth/row-level-security)
- [Storage](https://supabase.com/docs/guides/storage)

---

## 💡 次のステップ

セットアップが完了したら：

1. ✨ 写真をアップロードしてみる
2. 🔗 アルバムを共有してみる
3. 👥 別のユーザーで共同編集してみる
4. 🎨 [Figmaデザイン](https://www.figma.com/design/lPbJ0bzsWg0JfjucWuuoXu/Mantra-Photo-Album)と見比べる

楽しんでください！📸✨

