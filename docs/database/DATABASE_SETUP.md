# データベースセットアップガイド

## 問題

プロフィール設定画面で「プロフィールの保存に失敗しました」というエラーが発生する場合、Supabaseのデータベーステーブルが作成されていない可能性があります。

## 🚨 重要: SQLエラーが発生した場合

もし以下のようなエラーが表示された場合：
```
ERROR: 42703: column "user_id" does not exist
```

**最新版の`complete-setup.sql`を使用してください**（ストレージポリシーの問題を修正済み）

👉 詳細は [クイック修正ガイド](../development/QUICK_FIX_GUIDE.md) を参照

## 解決方法

以下の手順でデータベーススキーマを作成します。

### ステップ1: Supabaseダッシュボードにアクセス

1. [Supabase Dashboard](https://supabase.com/dashboard) を開く
2. プロジェクト一覧から該当のプロジェクトを選択
3. 左サイドバーから **SQL Editor** をクリック

### ステップ2: SQLスクリプトを実行

1. **New Query** ボタンをクリック
2. `supabase/sql/complete-setup.sql` ファイルの内容を全てコピー
3. SQL Editorにペースト
4. 右下の **Run** ボタンをクリック

**実行するSQL**: `/supabase/sql/complete-setup.sql`

### ステップ3: 実行結果を確認

**成功した場合**:
```
Success. No rows returned
```

**エラーが発生した場合**:
- エラーメッセージを確認
- 既にテーブルが存在する場合は問題なし（`already exists`エラー）

### ステップ4: テーブル作成を確認

1. 左サイドバーから **Table Editor** をクリック
2. 以下のテーブルが作成されていることを確認：
   - ✅ `profiles` - ユーザープロフィール
   - ✅ `albums` - アルバム
   - ✅ `photos` - 写真
   - ✅ `shares` - 共有リンク
   - ✅ `album_members` - アルバムメンバー
   - ✅ `comments` - コメント
   - ✅ `photo_tags` - 写真タグ

### ステップ5: ストレージバケットを確認

1. 左サイドバーから **Storage** をクリック
2. `photos` バケットが作成されていることを確認
3. もし作成されていない場合:
   - **Create a new bucket** をクリック
   - Name: `photos`
   - Public bucket: **オフ**（チェックを外す）
   - **Create bucket** をクリック

### ステップ6: アプリケーションをテスト

1. ブラウザでアプリケーションをリロード（F5）
2. プロフィール設定画面で情報を入力
3. 「完了してアルバムを始める」ボタンをクリック
4. **期待結果**: アルバム一覧ページにリダイレクトされる

## トラブルシューティング

### エラー: relation "public.profiles" does not exist

**原因**: テーブルが作成されていない

**解決方法**:
1. 上記のステップ2を実行
2. SQL実行時にエラーが出ていないか確認

### エラー: permission denied for table profiles

**原因**: RLS（Row Level Security）ポリシーが正しく設定されていない

**解決方法**:
1. Table Editor > `profiles` テーブルを開く
2. 右上の **...** メニュー > **Edit table** > **Enable RLS** を確認
3. もう一度 `complete-setup.sql` を実行

### エラー: duplicate key value violates unique constraint

**原因**: 既にプロフィールが存在する

**解決方法**:
1. Table Editor > `profiles` テーブルを開く
2. 該当ユーザーの行を削除
3. もう一度プロフィール設定を試す

### エラーが解決しない場合

**デバッグ手順**:

1. ブラウザの開発者ツール（F12）を開く
2. **Console** タブを確認
3. 以下のログを探す：
   ```
   プロフィール保存エラー（詳細）: { message: "...", ... }
   ```
4. エラーメッセージをコピーして確認

## 詳細な設定内容

### profiles テーブル構造

| カラム名 | 型 | 説明 |
|---------|-----|------|
| user_id | uuid | ユーザーID（主キー、auth.usersへの外部キー） |
| display_name | text | 表示名 |
| avatar_url | text | アバター画像のURL |
| bio | text | 自己紹介 |
| created_at | timestamptz | 作成日時 |
| updated_at | timestamptz | 更新日時 |

### RLS ポリシー

`profiles` テーブルには以下のRLSポリシーが設定されています：

- **SELECT**: 自分のプロフィールのみ参照可能
- **INSERT**: 自分のプロフィールのみ作成可能
- **UPDATE**: 自分のプロフィールのみ更新可能
- **DELETE**: 自分のプロフィールのみ削除可能

これにより、他のユーザーのプロフィールを見たり変更したりできないようになっています。

## 手動でテーブルを作成する場合

もしSQLスクリプトの実行がうまくいかない場合、以下の最小限のSQLで`profiles`テーブルだけを作成できます：

```sql
-- profilesテーブルを作成
CREATE TABLE public.profiles (
  user_id uuid PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  display_name text,
  avatar_url text,
  bio text,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);

-- RLS有効化
ALTER TABLE public.profiles ENABLE ROW LEVEL SECURITY;

-- 自分のプロフィールのみアクセス可能
CREATE POLICY profiles_all ON public.profiles
FOR ALL
USING (user_id = auth.uid())
WITH CHECK (user_id = auth.uid());
```

## 確認用SQL

データベースが正しくセットアップされているか確認するSQL：

```sql
-- テーブル一覧を確認
SELECT table_name 
FROM information_schema.tables 
WHERE table_schema = 'public'
ORDER BY table_name;

-- profilesテーブルの構造を確認
SELECT column_name, data_type 
FROM information_schema.columns 
WHERE table_schema = 'public' 
  AND table_name = 'profiles'
ORDER BY ordinal_position;

-- RLSポリシーを確認
SELECT schemaname, tablename, policyname, permissive, roles, cmd, qual
FROM pg_policies
WHERE tablename = 'profiles';

-- 自分のプロフィールを確認（ログイン後）
SELECT * FROM profiles WHERE user_id = auth.uid();
```

## 参考リンク

- [Supabase Row Level Security](https://supabase.com/docs/guides/auth/row-level-security)
- [Supabase SQL Editor](https://supabase.com/docs/guides/database/overview)
- [PostgreSQL データ型](https://www.postgresql.org/docs/current/datatype.html)

